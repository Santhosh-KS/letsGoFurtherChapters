...
// our email templates. This has a comment directive in the format
// `//go:embed <path>`.
...
// dynamic data for the templates as an emy parameter.

func (m Mailer) Send(recipient, templateFile string, data any) error {
	// User the ParseFS() method to pares the required template file from the embedded file system.
	...
	// Execute the named template "submect", passing in the dynamic data and storing the
	...
	// Follow the same pattern to execute the "plainBody" template and store the reuslt in
	// the plainBody variable.
	...
	// the SetBody() method to set the plain-text body, and the AddAlternative() method to set the HTML body.
	// It's important to note that AddAltenative() should always be called *after* SetBody()
	...
	// If there is a timeout, it will return a "dial tcp: io timeout" error
